...
const EditService: React.FC = () => {
      ...
      <div className="min-h-screen bg-gray-100">
        <div className="container mx-auto px-4 py-8">
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <p className="text-gray-600 mb-6">Je hebt geen rechten om diensten te bewerken.</p>
            <Link
              to={`/services/${id}`}
              className="inline-flex items-center space-x-2 bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors"
            >
              <ArrowLeft size={16} />
              <span>Terug naar Dienst</span>
            </Link>
          </div>
        </div>
      </div>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Dienst succesvol bijgewerkt!');
      navigate(`/services/${id}`);
    } catch (error) {
      setError(error instanceof Error ? error.message : 'Er is een fout opgetreden bij het bijwerken van de dienst.');
    } finally {
